...
class AdminController extends CI_Controller
{
	...
	public function parking_log()
	{
	    ...
	    $data['page_title'] = '<h2><center>Plaza Marcela</center></h2>
	...
	public function get_daily_trans()
	{
		...
		foreach ($data as $key => $value) {
			...
			if ($value['status'] == 0 || $value['remarks'] == 'PAID') {
				...
				/*end jay code*/
			$result['data'][] = [date('m-d-Y g:i:s a', strtotime($value['dateTimeIn'])), date('m-d-Y g:i:s a', strtotime($value['dateTimeout'])), $nn->name, $value['location'], $value['plateNumber'], $ww, $status];
		}
		echo json_encode($result);
	}
