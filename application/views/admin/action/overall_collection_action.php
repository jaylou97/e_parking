<script type="text/javascript">
	$(function () {
		$('#tbl_all_collection').DataTable({
              destroy:true,
			'ajax':'<?php echo base_url('get_overallcollection_list_data')?>',
			"autoWidth": false,
			"sScrollX": "120%",
			"sScrollXInner": "110%",
			"columnDefs":[
	...
	$('#dtstart').on('change', function(event){
		...
		if(dtstart <= dtend){
			...
			$('#tbl_all_collection').DataTable({
              destroy:true,
			'ajax':'<?php echo base_url()?>get_overallcollection_list_datavs2/'+dtstart+'/'+dtend,
			"autoWidth": false,
			"sScrollX": "120%",
			"sScrollXInner": "110%",
			"columnDefs":[
