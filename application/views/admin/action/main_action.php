...
function submitChange(paz){
  ...
  if(param == '1'){
    ...
    if(user == ''){
      ...
      Swal.fire('Invalid Input', 'Please input letters and numbers only', 'error')
    }else{
      $.post('change_username', {user: user}, function(msg) {
        if(msg == 'success'){
          // alertToust('success', 'Username is successfully changed.');
          Swal.fire('Username successfully changed', '', 'success')
          closeChange();
          setTimeout("window.location.href='LogOut'", 2000);
        }else if(msg == 'duplicate'){
          // alertToust('info', 'Please enter other username.');
          Swal.fire('Already Exist', 'Please enter another username', 'error')
          $('#change_user').val('');
        }
        else{
          // alertToust('info', msg);
  ...
  if(param == '2'){
    ...
    Swal.fire('Mismatch', 'Incorrect new and confirm password', 'error')
