...
class ReportViewController extends CI_Controller
{
	...
	public function report_incharge_endofshift()
	{
		...
		$emp_data = $this->AdminModel->getData_pis('pis.employee3', $inz);
		$data['mm_data'] = array('mall_name' => "Plaza Marcela", 'address' => "Corner Pamaong & Belderol Street,
		Cogon District, Tagbilaran City, Bohol", 'title' => "End of Shift: Cashier's Accountability Report (CAR)", 'cashier' => strtoupper($emp_data->name), 'date_end' => $ddt, 'page_theme' => "End Of Shift Report");
		$result2 = array();
		...
		foreach ($res as $key => $value) {
			...
			$trans_data = $this->ReportViewModel->getTransDataFromSync($value['outby'], $dt,  date('Y-m-d H:i:s', $dtendz), $value['amount']);
			$result2[] = [
				'v_type' => $ww,
				'coupon' => $value['coupon'],
		...
		foreach ($res2 as $key => $value) {
			$trans_data = $this->ReportViewModel->getTransDataFromSyncvs2($value['outby'], $dt,  date('Y-m-d H:i:s', $dtendz));
		...
		/*jay code*/
		$result5 = array();
		$res5 = $this->ReportViewModel->get_data_tblremittance($incharge_id, $dt, date('Y-m-d H:i:s', $dtendz));

		if (!empty($res5)) 
		{
			foreach ($res5 as $key => $value) 
				{
					$trans_data = $this->ReportViewModel->get_data_tblremittance2($value['p_attendant'], $dt,  date('Y-m-d H:i:s', $dtendz));
					$result5[] = [ 
						// 'p_attendant' => $value['p_attendant'],
						'r_time' => date('M-d-Y g:i:s A', strtotime($value['r_time'])),
						'r_amount' => $value['r_amount'],
						'r_amount_total' => $trans_data->r_amount,
						'1k' => $trans_data->onek,
						'5h' => $trans_data->fiveh,
						'1h' => $trans_data->oneh,
						'fifty' => $trans_data->fifty,
						'twenty' => $trans_data->twenty,
						'coins' => $trans_data->coins
					];
				}
		}
		else
	...
	/*--------jay controller*/
	public function pdf_print_ctrl()
	{
/*********************************************************************************************/
		$result = array();
		$persons = $this->AdminModel->getDatas('tbl_persons');
...
/***************************************************************************************************************/
		...
		$module = "Other Deduction";

		/*$get_date  =explode("_",$this->input->post('cutoff'));*/
		//$cutoff =$get_date[2];
        $cutoff ='March 28,2022';
		$filtered_date=date('M. d, Y', strtotime($cutoff));
		...
		$pdf->SetTitle($module.' Online Upload Summary - '.date('M. d, Y', strtotime($cutoff)));		
		$this->pdf_->reports_mainheader($pdf,"", $cutoff, $module); //MAIN HEADER upper left

		$pdf->SetFont('Arial','B',9);
		$header_width=Array(22,26,55,25,40,21);
		$pdf->SetWidths($header_width);

		//set line heigth. This is the height of each lines, not rows
		$pdf->SetLineHeight(5);
		$header_name=Array(
				"Vehicle Type",
				"Transaction No.",
		...
		$this->pdf_->reports_tableheader($pdf, "testing",$header_name,$header_width); /*Table header*/	
               ...
               /* ----------populate table rows----------*/
               $pdf->SetFont('Times','',9);
              /* $table_rows=$this->hr_mod->get_nav_uploaded_header();*/

		/*foreach($table_rows as $r)
		{*/
			/*$emp_id=$this->hr_mod->get_emp_id($r['ext_doc_no'],$r['order_bunit']);
			$get_emp_name=$this->simplify->get_employee_details($emp_id);

			foreach($get_emp_name as $n)
			{
				$name=$n['name'];
			}

			unset($rows);
			$amt_incl_vat=number_format($r['amt_incl_vat'],2);*/
			/*$action      ='<button class="btn btn-success btn-xs" onclick="view_details('."'".$r['doc_no']."'".','."'".$emp_id."'".')">view</button>';*/
		/*	$posting_date=date('M. d, Y',strtotime($r['posting_date']));
           
                        $ext_doc_no=explode("<br>",$this->hr_mod->get_ext_doc_no($r['doc_no']));*/
	...
	public function report_incharge_endofday()
	{
		...
		$ddt = date('F d, Y', strtotime($dt));
		$data['mm_data'] = array('mall_name' => "Plaza Marcela", 'address' => "Corner Pamaong & Belderol Street,
		Cogon District, Tagbilaran City, Bohol", 'title' => "End of Day: Cashiers' Accountability Summary (CAS)", 'date_end' => $ddt, 'page_theme' => "End Of Day Report");
		$result101 = array();
		$userdata = $this->ReportViewModel->getUsersData($dt, date('Y-m-d H:i:s', $dtendz));
		foreach ($userdata as $key => $valuex) {
			$inz = array('emp_id' => $valuex['outby']);
			$emp_data = $this->AdminModel->getData_pis('pis.employee3', $inz);
			$result2 = array();
			$location = $this->ReportViewModel->getInchargeTransLocation($valuex['outby'], $dt, date('Y-m-d H:i:s', $dtendz));
			$res = $this->ReportViewModel->getEndOfShiftData($valuex['outby'], $dt, date('Y-m-d H:i:s', $dtendz));
			...
			$result3 = array();
			$res2 = $this->ReportViewModel->getEndOfShiftDatavs2($valuex['outby'], $dt, date('Y-m-d H:i:s', $dtendz));
			foreach ($res2 as $key => $value) {
				$trans_data = $this->ReportViewModel->getTransDataFromSyncvs2($value['outby'], $dt,  date('Y-m-d H:i:s', $dtendz));
				$totz = $value['coupon_amount'] + $trans_data->penalty;
				// $totz = $value['coupon_amount'] + $trans_data->penalty;
	...
	public function report_incharge_daily()
	{
		...
		$ddt = date('F d, Y', strtotime($dt));
		$data['mm_data'] = array('mall_name' => "Plaza Marcela", 'address' => "Corner Pamaong & Belderol Street,
		Cogon District, Tagbilaran City, Bohol", 'title' => "Pay Parking Daily Statistics Report", 'date_end' => $ddt, 'page_theme' => "Daily Statistics Report");
		$result101 = array();
		$time_a = '7:00 am';
		$time_b = '8:30 am';
		for ($i = 1; $i <= 17; $i++) {
	...
	public function report_incharge_weekly()
	{
		...
		$data['mm_data'] = array('mall_name' => "Plaza Marcela", 'address' => "Corner Pamaong & Belderol Street,
		Cogon District, Tagbilaran City, Bohol", 'title' => "Pay Parking Weekly Statistics Report", 'date_start' => $f_dt, 'date_end' => $l_dt, 'page_theme' => "Weekly Statistics Report");
		$time_a = '7:00 am';
		$time_b = '8:30 am';
		for ($i = 1; $i <= 17; $i++) {
			$ta = date('H:i:s', strtotime($time_a));
	...
	public function report_incharge_monthly()
	{
		...
		$ldt = $m . ' ' . $d . ', ' . $y;
		$data['mm_data'] = array('mall_name' => "Plaza Marcela", 'address' => "Corner Pamaong & Belderol Street,
		Cogon District, Tagbilaran City, Bohol", 'title' => "Pay Parking Monthly Statistics Report", 'date_first' => $fdt, 'date_end' => $ldt, 'page_theme' => "Monthly Statistics Report");
		$time_a = '7:00 am';
		$time_b = '8:30 am';
		for ($i = 1; $i <= 17; $i++) {
			$ta = date('H:i:s', strtotime($time_a));
	...
	public function report_snyc_monthly_data()
	{
		...
		$data['mm_data'] = array('mall_name' => "Plaza Marcela", 'address' => "Corner Pamaong & Belderol Street,
		Cogon District, Tagbilaran City, Bohol", 'title' => "End of Day: Cashiers' Accountability Summary (CAS)", 'date_end' => $ddt, 'page_theme' => "Monthly Data Report");
		$result101 = array();
	...
	public function report_MonthlyRangeData()
	{
		...
		$data['mm_data'] = array('mall_name' => "Plaza Marcela", 'address' => "Corner Pamaong & Belderol Street,
		Cogon District, Tagbilaran City, Bohol", 'title' => "Monthly Range Data Report: Cashiers' Accountability Summary (CAS)", 'date_end' => $ddt, 'page_theme' => "Monthly Range Data Report");
		$result101 = array();
